...
const createBlog = async function (req, res) {
  try {
    ...
    if (!authorId) {
      return res
        .status(400)
        .send({ status: false, msg: "authorld is mandatory" });
    }
    if (!category) {
      return res
        .status(400)
    ...
    if (!authorchk) {
      return res
        .status(404)
        .send({ status: false, msg: "author id is not found in db" });
    }
...
const updateData = async (req, res) => {
  try {
    ...
    if (findBlogId.isDeleted)
      return res
        .status(404)
        .send({ status: false, msg: "Blog already been deleted" });
    let data = req.body;

    if (Object.keys(data).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "Data is required to update Blog" });
...
const deleteBlogs = async (req, res) => {
  try {
    ...
    if (Object.keys(data).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "Error!, no query found" });
    ...
    res
      ...
      .send({
        status: true,
        data: `${deletedBlogs.modifiedCount} blogs are deleted`,
      });
  } catch (err) {
    res.status(500).send({ status: false, error: err.message });
  }
