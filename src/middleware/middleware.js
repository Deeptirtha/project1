...
const authentication = function (req, res, next) {
  try {
    ...
    if (!token)
      return res
        .status(400)
        .send({ status: false, msg: "token must be present in header" });
    ...
    var decodedToken = jwt.verify(
      ...
      function (err, decodedToken) {
        if (err) {
          return res
            .status(401)
            .send({ status: false, msg: "invalid Token comming" });
        } else {
...
const authorization = async function (req, res, next) {
  try {
    ...
    if (!blog)
      return res
        .status(404)
        .send({ status: false, msg: "No such Blog is in dB" });
    if (blog.isDeleted)
      return res
        .status(404)
    ...
    if (authId != req.decodedToken.authorId)
      return res
        .status(403)
        .send({ status: false, msg: "you do not have authorization to this " });
    else {
      next();
    }
  } catch (err) {
